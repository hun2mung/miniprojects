...
class BackgroundWorker(QThread):    # PyQt5 스레드를 위한 클래스 존재
    procChanged = pyqtSignal(int)   # 커스텀 시그널(마우스 클릭같은 시그널 만드는 것)
    ...
    def run(self):  # thread.start() --> run() 대신 실행됨
        while self.working:
            if self.count <= MAX:
                self.procChanged.emit(self.count)   # 시그널 내보냄
                self.count += 1             # 값 증가만 // 업무프로세스 동작하는 위치
                time.sleep(0.0001)              # 세밀하게 주면 GUI 처리 제대로 못함
            else:
...
class qtApp(QWidget):
    ...
    def btnStartClicked(self):
        self.worker.start() # 스레드 클래스 아래 run() 실행
